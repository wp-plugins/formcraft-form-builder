...
{
	var height = Math.max(jQuery(event.target).outerHeight(),0);
	if (height>0)
	{
...
jQuery(document).mouseup(function (e)
{
		...
		|| containerOuter.has(e.target).length !== 0)
	{
			...
			&& containerInner.has(e.target).length === 0)
		{
			...
			jQuery('.form-element.options-true .icon-minus').trigger('click');
		}
	}
});
toastr.options = {
...
.controller('FormController', function($scope, $locale, $http) {
	...
	$scope.FormElements(); /* Load Form Styling */
	...
	$scope.fieldHTMLTemplate.oneLineText = "<div class='oneLineText-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><input type='text' name='{{element.elementDefaults.identifier}}' data-min-char='{{element.elementDefaults.Validation.minChar}}' data-max-char='{{element.elementDefaults.Validation.maxChar}}' data-val-type='{{element.elementDefaults.Validation.allowed}}' data-is-required='{{element.elementDefaults.required}}' data-allow-spaces='{{element.elementDefaults.Validation.spaces}}' class='validation-lenient'></div></div>";
	...
	$scope.fieldHTMLTemplate.email = "<div class='email-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><input type='text' data-val-type='email' data-is-required='{{element.elementDefaults.required}}' name='{{element.elementDefaults.identifier}}' class='validation-lenient'></div></div>";
	...
	$scope.fieldHTMLTemplate.textarea = "<div class='textarea-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><textarea class='validation-lenient' name='{{element.elementDefaults.identifier}}' value='' rows='{{element.elementDefaults.field_height}}' data-min-char='{{element.elementDefaults.Validation.minChar}}' data-max-char='{{element.elementDefaults.Validation.maxChar}}' data-is-required='{{element.elementDefaults.required}}'></textarea></div></div>";
	...
	$scope.fieldHTMLTemplate.checkbox = "<div class='checkbox-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><label ng-repeat='opt in element.elementDefaults.options_list_show'><input type='{{element.elementDefaults.allow_multiple}}' data-is-required='{{element.elementDefaults.required}}' name='{{element.elementDefaults.identifier}}[]' value='{{opt.value}}' class='validation-lenient'><span compile='opt.show'></span></label></div></div>";
	...
	$scope.fieldHTMLTemplate.dropdown = "<div class='dropdown-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><select data-is-required='{{element.elementDefaults.required}}' class='validation-lenient' name='{{element.elementDefaults.identifier}}'><option value='{{opt.value}}' ng-repeat='opt in element.elementDefaults.options_list_show'>{{opt.show}}</option></select></div></div>";
	...
	$scope.fieldHTMLTemplate.datepicker = "<div class='datepicker-cover field-cover'><span compile='element.elementDefaults.main_label'></span><div><input type='text' class='validation-lenient' data-is-required='{{element.elementDefaults.required}}' datepicker data-date-format='{{element.elementDefaults.dateFormat}}' data-date-min='{{element.elementDefaults.minDate}}' data-date-min-alt='{{element.elementDefaults.minDateAlt}}' data-date-max-alt='{{element.elementDefaults.maxDateAlt}}' data-date-max='{{element.elementDefaults.maxDate}}' data-date-lang='{{element.elementDefaults.dateLang}}' name='{{element.elementDefaults.identifier}}' ng-model='temp'></div></div>";
